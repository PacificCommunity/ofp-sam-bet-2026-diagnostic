...
if [ ! -x ./mfclo64 ]; then
  ...
  echo "Place mfclo64 in this directory." >&2
  exit 1
fi
...
./mfclo64 bet.frq 00.dm-fixed.par 01.par -file - <<PHASE1
...
if [ "$seed23_apply" = true ]; then
  if ./mfclo64 bet.frq 01.par seed23-phase02-probe.par -file - <<SEED23_PHASE2_PROBE
  1 1 100
  1 50 0
  2 113 0
  1 190 1
...
if [ "$seed23_apply" = true ]; then
  if ./mfclo64 bet.frq 04.par seed23-phase05-probe.par -file - <<SEED23_PHASE5_PROBE
  -100000 1 1
  -100000 2 1
  -100000 3 1
  -100000 4 1
...
./mfclo64 bet.frq 09.par 10.par -file - <<PHASE10_NO_TAU_EST
  1 1 10000
  1 50 $phase10_11_convergence
  1 121 0
PHASE10_NO_TAU_EST

./mfclo64 bet.frq 10.par 11.par -file - <<PHASE11_NO_TAU_EST
  1 1 5000
  1 50 $phase10_11_convergence
  1 121 0
  1 246 1
