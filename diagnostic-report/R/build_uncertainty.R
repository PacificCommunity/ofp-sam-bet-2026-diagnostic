...
hessian_dir <- Sys.getenv("DIAGNOSTIC_HESSIAN_DIR", file.path(model_dir, "hessian"))
if (!grepl("^/", hessian_dir)) hessian_dir <- file.path(repo_root, hessian_dir)
hessian_dir <- normalizePath(hessian_dir, mustWork = TRUE)
variance_file <- if (length(args) >= 3L) args[[3L]] else Sys.getenv(
  "DIAGNOSTIC_VARIANCE_FILE", file.path(model_dir, "bet.var")
)
if (!grepl("^/", variance_file)) variance_file <- file.path(repo_root, variance_file)
report_dpi <- suppressWarnings(as.integer(Sys.getenv("DIAGNOSTIC_REPORT_DPI", "400")))
if (!is.finite(report_dpi) || report_dpi < 150L) report_dpi <- 400L
...
spawning <- spawning[!duplicated(spawning$period), , drop = FALSE]
...
recruitment <- recruitment[!duplicated(recruitment$period), , drop = FALSE]
...
dynamic <- dynamic[!duplicated(dynamic$period), , drop = FALSE]
...
terminal_period <- max(spawning$period, na.rm = TRUE)
terminal_mgmt <- pick_contrast(paste0(
  "^adult_rbio\\(", terminal_period, "\\) - average_adult_rbio_noeff\\(40_periods\\) "
))
...
ggsave(file.path(figure_dir, "hessian-time-series.png"), combined, width = 13, height = 10, dpi = report_dpi, bg = "white")
...
p_status <- ggplot(status_plot, aes(estimate, quantity)) +
  ...
  theme_bet()
ggsave(file.path(figure_dir, "management-status-uncertainty.png"), p_status, width = 9, height = 4.5, dpi = report_dpi, bg = "white")
...
if (nrow(numbers)) {
  ...
  ggsave(file.path(figure_dir, "terminal-abundance-at-age-uncertainty.png"), p_numbers, width = 12, height = 8, dpi = report_dpi, bg = "white")
}

hessian_candidates <- list.files(hessian_dir, pattern = "^hessian_info\\.rds$", recursive = TRUE, full.names = TRUE)
hessian_file <- if (length(hessian_candidates)) hessian_candidates[[1L]] else ""
fit_file <- file.path(model_dir, "fit-summary.csv")
...
if (file.exists(hessian_file)) {
  ...
  ggsave(file.path(figure_dir, "regional-age-length-growth-uncertainty.png"), combined_growth_plot, width = 12, height = 10.2, dpi = report_dpi, bg = "white")
